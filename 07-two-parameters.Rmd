...
<!-- 1) Last section is called Virtues. Adding a discussion of the Four Virtues including their pictures. A paragraph or two about each. Only need to do this for the height example, not the pennies. precepttor tables, pupulation, figures -->
...
The workaround to having a *single* sample was to perform *bootstrap resampling with replacement* from the single sample. We did this in the resampling activity in Section \@ref(resampling-tactile) where we focused on the mean year of minting of pennies. We used pieces of paper representing the original sample of 50 pennies from the bank and resampled them with replacement from a hat. We had 35 of our friends perform this activity and visualized the resulting 35 sample means $\overline{x}$ in a histogram. 
